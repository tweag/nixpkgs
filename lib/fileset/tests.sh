...
# The first argument is an attribute set, but it doesn't support passing arbitrary attributes
expectFailure 'trace { unknown = true; } ./.' 'lib.fileset.trace: The first argument must be \{ \}. In the future this can allow configuration of the trace format.'
...
expectTrace '{}' '(unions (mapAttrsToList (n: _: ./. + "/${n}") (removeAttrs (builtins.readDir ./.) [ "0" ]))) null' "$expectedTrace"
rm -rf -- *
