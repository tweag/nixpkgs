...
in /* No rec! Add dependencies on this file at the top. */ {
  ...
  */
  ...
  - `subpath`: The normalised subpath string (see `lib.path.subpath.normalise) that when appended (see `lib.path.append`) to `root` returns the original path.
  ...
  Example:
    ...
    splitRoot /.
    => { root = /.; subpath = "./."; }
    # Nix neutralises `..` path components for all paths automatically
    splitRoot /foo/../bar
    => { root = /.; subpath = "./bar"; }
    splitRoot "/foo/bar"
    => <error>
